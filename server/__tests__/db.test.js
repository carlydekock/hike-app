...
beforeAll(async () => {
  ...
  await db.query('CREATE TABLE hikes_list (id SERIAL PRIMARY KEY, user_id INT NOT NULL, name VARCHAR(255) NOT NULL, description TEXT NOT NULL, length VARCHAR(255) NOT NULL, elevation_gain DECIMAL NOT NULL, time VARCHAR(255), keywords VARCHAR(255), latitude DECIMAL NOT NULL, longitude DECIMAL NOT NULL);');
...
beforeEach(async () => {
  ...
  await db.query('INSERT INTO hikes_list (user_id, name, description, length, elevation_gain, time, keywords, latitude, longitude) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)', [1, 'hike1', 'Testing description', '5 miles', 1000, '5 hours', 'mountain', 1.000, 1.500]);
});
